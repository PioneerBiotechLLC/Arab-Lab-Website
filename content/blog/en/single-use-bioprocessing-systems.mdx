...
Single-use has limits. At very large scale, the maximum practical size of disposable bags and bioreactors can become a constraint {{VERIFY: current typical maximum working volumes of single-use bioreactors}}. For a single product running continuously at high volume, the recurring cost of disposables can outweigh the cleaning savings. Some processes involve solvents or conditions that disposable polymers do not tolerate well.
...
Suppliers generally provide extractables data under standardised protocols {{VERIFY: current references for single-use extractables testing, e.g. USP <665> and <1665> and the BPOG protocol}}. The manufacturer then assesses risk for each component based on contact time, temperature, the solution in contact and the proximity to the final product. Components close to the final product, such as final filters and fill assemblies, attract the most scrutiny.

## Integrity and sterility assurance

A single-use system replaces a validated clean piece of steel with an assembly that is sterilised by the supplier, usually by gamma irradiation or another validated method {{VERIFY: common sterilisation methods and the shift from gamma to X-ray for single-use components}}. The user must be confident that the assembly arrives sterile and stays intact through handling and use.
