...
2. **Hold the pipette near vertical when aspirating.** Tilting changes the height of liquid pulled in and the volume aspirated {{VERIFY: recommended maximum angle from vertical during aspiration, e.g. within 20 degrees}}.
3. **Immerse to the right depth.** Too shallow draws in air; too deep coats the outside of the tip with liquid that is then carried over {{VERIFY: recommended immersion depths by volume range, e.g. 1–2 mm for small volumes and 3–6 mm for larger volumes}}.
...
ISO 8655 specifies requirements and test methods for piston-operated volumetric apparatus, including single- and multichannel pipettes {{VERIFY: parts and current edition of ISO 8655 and the main changes in the latest revision}}. The reference method is **gravimetric**: water is dispensed onto a sensitive balance, the mass is converted to volume using a correction factor for temperature and air pressure, and the results are compared with maximum permissible errors for accuracy and precision.
...
Electronic pipettes reduce variation caused by differences in thumb pressure and speed, and they reduce the physical strain of repetitive pipetting, which is a genuine occupational health concern in busy laboratories {{VERIFY: whether to cite specific ergonomic data on repetitive strain from pipetting}}. Automated systems make sense when throughput is high, when the same protocol runs repeatedly, or when manual pipetting is a documented source of error.
