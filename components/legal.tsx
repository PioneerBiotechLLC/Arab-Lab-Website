...
import { legalDocs, type Block, type LegalDoc } from '@/lib/legal'
...
function BlockView({ block }: { block: Block }) {
  ...
  if ('list' in block) return <ul className="grid gap-3 pl-5 text-base leading-7 text-muted-foreground marker:text-orange">{block.list.map((item) => <li key={item} className="pl-1 list-disc">{inline(item)}</li>)}</ul>
...
export function LegalPage({ doc }: { doc: LegalDoc }) {
  ...
  return <PageShell breadcrumbs={[{ name: 'Home', href: '/' }, { name: doc.title, href: `/${doc.slug}` }]} title={doc.title} intro={doc.intro}>
    <PageSection>
      <div className="grid gap-12 lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-20">
        <nav aria-label="Contents" className="lg:sticky lg:top-28 lg:self-start">
          <p className="font-mono text-xs text-muted-foreground">Last updated {doc.updated}</p>
          <ol className="mt-6 grid border-l border-border text-sm">
            {doc.sections.map((s) => <li key={s.id}><a href={`#${s.id}`} className="-ml-px block border-l border-transparent py-2 pl-4 text-muted-foreground hover:border-orange hover:text-ink">{s.title}</a></li>)}
          </ol>
        </nav>
    ...
    <PageSection className="bg-paper">
      <p className="font-mono text-xs text-muted-foreground">Also read</p>
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {others.map((d) => <Link key={d.slug} href={`/${d.slug}`} className="group flex items-center justify-between gap-6 rounded-2xl border border-line bg-white p-6 shadow-card hover:border-orange/60">
