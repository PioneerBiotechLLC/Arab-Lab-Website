  ...
  return <PageShell breadcrumbs={[{ name: 'Home', href: '/' }, { name: 'Brands', href: '/brands' }, { name: brand.name, href: `/brands/${brand.slug}` }]}
    ...
    {content && <PageSection className="bg-paper">
      <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
        ...
        <div className="grid gap-4">
          {solves.length > 0 && <div className="rounded-3xl border border-border bg-white p-8 shadow-card"><h3 className="font-heading text-xl font-bold text-ink">Where {brand.name} fits</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">The Arab Lab {solves.length === 1 ? 'solution' : 'solutions'} this partner serves:</p><ul className="mt-4 grid gap-2">{solves.map((item) => <li key={item.id}><Link href={`/solutions#${item.id}`} className="group flex items-center justify-between gap-4 py-2 font-heading font-semibold text-ink hover:text-orange">{item.title}<ArrowUpRight className="size-4 shrink-0 text-orange transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></Link></li>)}</ul></div>}
          <div className="rounded-3xl border border-border bg-white p-8 shadow-card"><h3 className="font-heading text-xl font-bold text-ink">Available across the UAE, Saudi Arabia and Egypt</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Requests for {brand.name} are handled by the Arab Lab office that covers your country.</p><ul className="mt-4 grid gap-1">{offices.map((office) => <li key={office.slug}><Link href="/locations" className="flex items-center gap-3 py-2 text-sm font-medium text-ink hover:text-orange"><MapPin aria-hidden className="size-4 text-orange" />{office.name}, {office.country}{office.headquarters ? ' · HQ' : ''}</Link></li>)}</ul></div>
        </div>
      </div>
    </PageSection>}
    {content && <PageSection>
