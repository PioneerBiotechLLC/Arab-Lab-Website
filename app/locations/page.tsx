...
export default function LocationsPage() {
  return <PageShell eyebrow="Locations" title="Regional presence, local response." intro={`${offices.length} offices keep Arab Lab close to laboratory teams across ${markets.join(', ')}. Headquarters: Ras Al Khaimah, UAE.`}
    ...
    {markets.map((country, index) => {
      ...
      return <PageSection key={country} className={index % 2 ? 'bg-paper' : ''}>
        <SectionIntro eyebrow={`0${index + 1} / ${country}`} title={country} intro={`${local.length} ${local.length === 1 ? 'office' : 'offices'}`} />
    ...
    <DarkBand overlap eyebrow="Who you’ll reach" title="The right department, wherever you are." intro="Every request is routed to the Arab Lab department that owns it.">
